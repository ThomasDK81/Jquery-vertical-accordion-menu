...
class dc_jqaccordion_widget extends WP_Widget {
	...
	function dc_jqaccordion_widget() {
		...
		$name        = __( 'jQuery Accordion Menu', 'dcjq-accordion' );
		$desc        = __( 'Vertical Accordion From Custom Menus', 'dcjq-accordion' );
		...
		parent::__construct( 'nav_menu', __( 'Custom Menu', 'dcjq-accordion' ), $widget_ops );
	...
	function form( $instance ) {
		...
		if ( ! isset( $instance['autoClose'] ) ) {
			$instance['autoClose'] = 'false';
		}
		if ( ! isset( $instance['saveState'] ) ) {
			$instance['saveState'] = 'false';
		}
		if ( ! isset( $instance['menuClose'] ) ) {
			$instance['menuClose'] = 'false';
		}
		if ( ! isset( $instance['disableLink'] ) ) {
			$instance['disableLink'] = 'false';
		}
		if ( ! isset( $instance['classDisable'] ) ) {
			$instance['classDisable'] = '';
		}
		if ( ! isset( $instance['classMenu'] ) ) {
			$instance['classMenu'] = '';
		}
		if ( ! isset( $instance['autoExpand'] ) ) {
			$instance['autoExpand'] = 'false';
		}
		...
		if ( !$menus ) {
			echo '<p>'. sprintf( __('No menus have been created yet. <a href="%s">Create some</a>.', 'dcjq-accordion' ), admin_url('nav-menus.php') ) .'</p>';
			return;
		}
		?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'dcjq-accordion' ) ?></label>
			<input type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $title; ?>"  size="20" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('nav_menu'); ?>"><?php _e('Select Menu:', 'dcjq-accordion' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('classMenu'); ?>"><?php _e('Menu Class:', 'dcjq-accordion') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('classDisable'); ?>"><?php _e('Disable Class:', 'dcjq-accordion') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('hoverDelay'); ?>"><?php _e('Hover Delay:', 'dcjq-accordion'); ?>
				<select name="<?php echo $this->get_field_name('hoverDelay'); ?>" id="<?php echo $this->get_field_id('hoverDelay'); ?>" >
					<option value='0' <?php selected( $hoverDelay, '0'); ?> ><?php _e('No Delay', 'dcjq-accordion'); ?></option>
					<option value='200' <?php selected( $hoverDelay, '200'); ?> ><?php _e('0.2 sec', 'dcjq-accordion'); ?></option>
					<option value='400' <?php selected( $hoverDelay, '400'); ?> ><?php _e('0.4 sec', 'dcjq-accordion'); ?></option>
					<option value='600' <?php selected( $hoverDelay, '600'); ?> ><?php _e('0.6 sec', 'dcjq-accordion'); ?></option>
					<option value='800' <?php selected( $hoverDelay, '800'); ?> ><?php _e('0.8 sec', 'dcjq-accordion'); ?></option>
					<option value='1000' <?php selected( $hoverDelay, '1000'); ?> ><?php _e('1.0 sec', 'dcjq-accordion'); ?></option>
				</select>
			</label>
		</p>
		<p><label for="<?php echo $this->get_field_id('speed'); ?>"><?php _e('Animation Speed:', 'dcjq-accordion'); ?>
				<select name="<?php echo $this->get_field_name('speed'); ?>" id="<?php echo $this->get_field_id('speed'); ?>" >
					<option value='slow' <?php selected( $speed, 'slow'); ?> ><?php _e( 'Slow', 'dcjq-accordion' ); ?></option>
					<option value='normal' <?php selected( $speed, 'normal'); ?> ><?php _e( 'Normal', 'dcjq-accordion' ); ?></option>
					<option value='fast' <?php selected( $speed, 'fast'); ?> ><?php _e( 'Fast', 'dcjq-accordion' ); ?></option>
				</select>
			</label>
		</p>
		<p><label for="<?php echo $this->get_field_id('skin'); ?>"><?php _e('Skin:', 'dcjq-accordion'); ?>  <?php
				...
				echo "<option value='no-theme' ".selected( $skin, 'no-theme', false).">" . __( 'No theme', 'dcjq-accordion' ) . "</option>";
